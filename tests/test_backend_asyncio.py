"""
Tests for the asyncio loop. We use the offscreem canvas to test it with.
"""
...
class AsyncioLoop(loop.__class__):
    pass
...
class AsyncioCanvas(RenderCanvas):
...
class AsyncioHelper(NativeHelper):
    def close_canvas(self, canvas):
        canvas.close()


@pytest.mark.parametrize("func", BACKEND_TEST_FUNCS)
def test_backend_asyncio(func):
    func(AsyncioCanvas, loop, AsyncioHelper())
