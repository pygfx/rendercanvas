/*************************************************************************************************
  ...
  This module implements the rendercanvas event spec for canvases in a browser.
  It implements observers and event listeners and converts these into
  rendercanvas-specific event dicts. The code is written with little assumptions
  about the applications, so that it can be applied in different backends, e.g.
  pyodide, anywidget, remote-browser.

  Code that loads the script should avoid loading it in the global scope. Either
  by putting it in a ``<script type='module>``, or wrapping it in a ``(() => {\n{JS}\n})();``

  This code is formatted and linted with standardjs (quite similar to Ruff in
  Python). It adheres to common JS practices, but it probably shows that
  rendercanvas is a Python project, e.g. the underscore prefix for private
  attributes.
